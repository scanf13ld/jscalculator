...
if ($_POST){
  ...
  if($dur == "weekly"){
  ...
  else{ //just updating a single event

      $stmt = $mysqli->prepare("UPDATE events SET month = ?, year = ?, day = ?, title = ?, tag_id = ?, time_event = ?, dur = ? WHERE events.event_id =?");
      //deleteEvent($event_id);
      //$stmt = $mysqli->prepare("insert into events (user_id, month, year, day, title, tag_id, dur, time_event) values (?, ?, ?, ?, ?, ?, ?, ?)");
      ...
      //$stmt->bind_param('siisissi', $_SESSION["username"],$m, $y, $d, $t, $tag, $dur, $time);
      $stmt->bind_param('iiisissi', $m, $y, $d, $t, $tag, $time, $dur, $event_id);

      $stmt->execute();

      $stmt->close();

      echo json_encode(htmlentities("Event successfully edited."));
    }
  }
